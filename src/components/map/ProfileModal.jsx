...
export default function ProfileModal({ user, onClose, onCollaborate }) {
    ...
    return (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[9999] p-4 animate-fade-in" onClick={onClose}>
            <div
                className="bg-slate-900 border border-white/10 rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-2xl relative"
                onClick={(e) => e.stopPropagation()}
            >
                {/* Header */}
                <div className="relative h-24 bg-gradient-to-r from-slate-900 to-slate-800 border-b border-white/5">
                    <div className={`absolute inset-0 opacity-20 ${roleAttrs.bg}`}></div>
                    <button
                        onClick={onClose}
                        className="absolute top-4 right-4 p-2 bg-black/20 hover:bg-black/40 rounded-full text-white/50 hover:text-white transition z-10"
                    >
                        <X className="h-5 w-5" />
                    </button>

                    <div className="absolute -bottom-8 left-8 flex items-end gap-5">
                        <div className="w-20 h-20 rounded-2xl bg-slate-900 border-4 border-slate-900 flex items-center justify-center text-3xl shadow-xl">
                            {roleAttrs.icon}
                        </div>
                        <div className="mb-2">
                            ...
                            <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider border ${roleAttrs.bg} ${roleAttrs.color} ${roleAttrs.border}`}>
                                {user.village_role}
                            </span>
                        </div>
                    </div>
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                {user.skills.map((skill, idx) => (
                                    <div key={idx} className="bg-slate-800/50 border border-white/5 rounded-lg p-2.5 hover:border-cyan-500/30 transition-colors">
                                        <p className="font-semibold text-slate-200 text-sm">{skill.skill_name}</p>
                            ...
                            <div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {user.languages.map((lang, idx) => (
                                        <span key={idx} className="bg-purple-500/10 text-purple-300 border border-purple-500/20 px-2.5 py-1 rounded-full text-[10px] font-medium">
                                            {lang.language}
                                        </span>
                                    ))}
                                </div>
                            ...
                            <div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {user.passions.map((passion, idx) => (
                                        <span key={idx} className="bg-pink-500/10 text-pink-300 border border-pink-500/20 px-2.5 py-1 rounded-full text-[10px] font-medium">
                                            {passion}
                                        </span>
                                    ))}
                                </div>
                        ...
                        <div>
                            ...
                            <div className="space-y-2">
                                {user.projects.map((project, idx) => (
                                    <div key={idx} className="bg-gradient-to-br from-slate-800/80 to-slate-900 border border-white/5 rounded-xl p-4 hover:border-green-500/30 transition-all group">
                                        <div className="flex justify-between items-start">
                                            <h4 className="font-bold text-white text-base group-hover:text-green-400 transition-colors">{project.title}</h4>
                    ...
                    {onCollaborate && (
                        <div className="pt-2">
                            <button
                                ...
                                className="w-full relative overflow-hidden group bg-gradient-to-r from-cyan-600 to-blue-600 text-white py-3 px-6 rounded-xl font-bold uppercase tracking-wide shadow-lg hover:shadow-cyan-500/25 transition-all transform hover:-translate-y-1 text-sm"
                            >
